...
def acquire_singleton_lock() -> bool:
    ...
    try:
        ...
        print(
            "[WARN] Another local instance may be active, proceeding in serverless mode...",
            flush=True,
        )
        return True
...
ADMIN_SECRET_KEY = os.environ.get(
    "ADMIN_SECRET_KEY", "channeldna-secret-admin-key-2026"
)


async def handle_health(request):
    return web.Response(
        text="RoughCut Discord Bot 24/7 is Live!",
...
async def handle_sync_profile(request):
    try:
        ...
        print(
            f"[Cloud Profile Synced] Saved '{prof.channel_name}' ({prof.profile_type}) to Render DB."
        )
        return web.json_response(
            {"success": True, "profile_id": prof.profile_id},
            headers={"Access-Control-Allow-Origin": "*"},
        )
...
async def handle_issue_passcode(request):
    try:
        ...
        print(
            f"[Cloud Binding Synced] Registered '{st_name}' ({ch_id}, passcode: {passcode}) on Render DB."
        )
        return web.json_response(
            {"success": True, "passcode": passcode},
            headers={"Access-Control-Allow-Origin": "*"},
        )
    except Exception as e:
        return web.json_response(
...
async def handle_unbind_streamer(request):
    try:
        ...
        return web.json_response(
            {"success": res}, headers={"Access-Control-Allow-Origin": "*"}
        )
    except Exception as e:
        return web.json_response(
            {"success": False, "error": str(e)},
...
async def handle_list_profiles(request):
    try:
        with db._session() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT profile_id, channel_name, avg_shot_length, tension_interval, silence_tolerance, highlight_rms_threshold, profile_type, youtube_url, chzzk_url, updated_at FROM channel_profiles;"
            )
            rows = cur.fetchall()
            profiles = [dict(r) for r in rows]

            cur.execute(
                "SELECT channel_id, streamer_name, target_dna_profile, passcode, is_bound, created_at, bound_at FROM streamer_bindings;"
            )
            b_rows = cur.fetchall()
            bindings = [dict(b) for b in b_rows]

        return web.json_response(
            {"success": True, "profiles": profiles, "bindings": bindings},
            headers={"Access-Control-Allow-Origin": "*"},
        )
    except Exception as e:
        return web.json_response(
...
async def handle_add_credit(request):
    ...
    try:
        ...
        if not user_id or amount <= 0:
            return web.json_response(
                {"success": False, "error": "Invalid user_id or amount"},
                status=400,
                headers={"Access-Control-Allow-Origin": "*"},
            )

        new_bal = db.add_user_credits(
            int(user_id), int(amount), reason=reason, order_id=order_id
        )
        return web.json_response(
            {
                "success": True,
    ...
    try:
        ...
        if state == "4" and discord_user_id.isdigit():
            ...
            if credits_to_add > 0:
                ...
                print(
                    f"[PayApp] Successfully added {credits_to_add} credits to user {discord_user_id}. Order: {mul_no}"
                )
    ...
    while True:
        ...
        try:
            ...
            if user:
                ...
                print(
                    f"[OK] Successfully delivered package to Discord User: {msg.user_id}"
                )
        except Exception as e:
...
async def cmd_status(interaction: discord.Interaction):
    ...
    for b in bindings:
        ...
        status_str = (
            f"인증완료 (수신자: {discord_id})"
            if is_bound
            else f"대기중 (암호: {passcode})"
        )
        lines.append(
            f"• {st_name} ({ch_id[:8]}...): {status_str} | 최근VOD: {last_vod}"
        )
...
@app_commands.describe(
    대상유저="크레딧을 지급할 디스코드 유저",
    수량="지급할 크레딧 수량",
    사유="지급 사유",
)
async def cmd_give_credit(
    ...
    if 수량 <= 0:
        await interaction.response.send_message(
            "❌ 수량은 1 이상이어야 합니다.", ephemeral=True
        )
        return
...
@app_commands.default_permissions(administrator=True)
@app_commands.describe(
    대상유저="크레딧을 차감할 디스코드 유저",
    수량="차감할 크레딧 수량",
    사유="차감 사유",
)
async def cmd_deduct_credit(
    interaction: discord.Interaction,
    대상유저: discord.User,
    수량: int,
    사유: str = "관리자 수동 차감",
    ...
    if not success:
        await interaction.response.send_message(
            f"❌ 잔액이 부족합니다. (현재 잔액: `{new_bal}`개)", ephemeral=True
        )
        return
    await interaction.response.send_message(
...
class ProfileRegistrationModal(
    discord.ui.Modal, title="새 스트리머 발화 프로필 등록 (3+3 분석)"
):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        solo_urls = [
            u.strip() for u in self.solo_links_input.value.split("\n") if u.strip()
        ]
        collab_urls = [
            u.strip() for u in self.collab_links_input.value.split("\n") if u.strip()
        ]
...
class ProfileSelectDropdown(discord.ui.Select):
    def __init__(self, profiles: list[dict[str, Any]], parent_view: Any):
        ...
        super().__init__(
            placeholder="🎯 적용할 스트리머 발화 프로필을 선택하세요",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.parent_view.discord_user_id:
            await interaction.response.send_message(
                "❌ 본인의 요청에서만 선택할 수 있습니다.", ephemeral=True
            )
            return
...
class ProfileAndModeSelectionView(discord.ui.View):
    def __init__(
        ...
        # Step 2: Buttons
        self.solo_btn = discord.ui.Button(
            label="🎤 솔로 방송 모드",
            style=discord.ButtonStyle.primary,
            custom_id="btn_solo_exec",
            disabled=False,
        )
        self.collab_btn = discord.ui.Button(
            label="👥 합방 / 콜라보 모드",
            style=discord.ButtonStyle.success,
            custom_id="btn_collab_exec",
            disabled=False,
        )
    ...
    async def _execute(self, interaction: discord.Interaction, mode: str, mode_kr: str):
        if interaction.user.id != self.discord_user_id:
            await interaction.response.send_message(
                "❌ 본인의 요청에서만 실행할 수 있습니다.", ephemeral=True
            )
            return
        ...
        if summary["is_vip"] and summary["free_remaining_today"] > 0:
            ...
            db.record_daily_usage(
                self.discord_user_id, is_free_quota=True, video_no=self.video_no
            )
        else:
            # Must consume 1 credit
            ok, rem = db.deduct_user_credit(
                self.discord_user_id, reason=f"가편집 VOD: {self.video_no}"
            )
            if not ok:
                await interaction.response.send_message(
                    f"⚠️ **[크레딧 부족 안내]**\n"
                ...
                return
            db.record_daily_usage(
                self.discord_user_id, is_free_quota=False, video_no=self.video_no
            )
...
async def cmd_my_profiles(interaction: discord.Interaction):
    ...
    for i, p in enumerate(profiles, start=1):
        ...
        lines.append(
            f"**{i}. {p['profile_name']}** (ID: `{p['profile_id']}`)\n   • 솔로 ASL: `{s_asl}s` | 합방 ASL: `{c_asl}s`"
        )
...
async def cmd_delete_profile(interaction: discord.Interaction, 프로필id: str):
    success = db.delete_user_profile(
        프로필id.strip(), discord_user_id=interaction.user.id
    )
    if success:
        await interaction.response.send_message(
            f"✅ 프로필(`{프로필id}`)이 정상적으로 삭제되었습니다.", ephemeral=True
        )
    else:
        await interaction.response.send_message(
            f"❌ 해당 프로필을 찾을 수 없거나 삭제 권한이 없습니다.", ephemeral=True
        )
...
async def cmd_my_credits(interaction: discord.Interaction):
    ...
    vip_badge = (
        "👑 VIP 스트리머 (1일 2회 무료 제공)"
        if summary["is_vip"]
        else "👤 일반 크리에이터"
    )
    free_info = (
        f"• **오늘 남은 무료 가편집:** `{summary['free_remaining_today']} / 2회`"
        if summary["is_vip"]
        else "• **일일 무료 혜택:** 일반 회원 (크레딧 전용)"
    )
...
@app_commands.describe(amount="충전할 수량 (숫자 미입력 시 요금표 안내가 출력됩니다)")
async def cmd_charge_credits(interaction: discord.Interaction, amount: int = None):
    ...
    if amount < 1:
        await interaction.response.send_message(
            "❌ 최소 1크레딧 이상 충전해야 합니다.", ephemeral=True
        )
        return
    ...
    if price < 3000:
        await interaction.response.send_message(
            "❌ 페이앱 결제 최소 금액은 3,000원입니다.", ephemeral=True
        )
        return
...
@app_commands.describe(
    치지직_링크="치지직 다시보기 영상 링크 (예: https://chzzk.naver.com/video/1049281)"
)
async def cmd_manual_roughcut(interaction: discord.Interaction, 치지직_링크: str):
    ...
    if not v_no:
        await interaction.response.send_message(
            "❌ 올바른 치지직 다시보기 영상 링크를 입력해 주세요.", ephemeral=True
        )
        return
...
class ModeSelectionView(discord.ui.View):
    ...
    async def collab_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        await self._handle_selection(interaction, "collab", "합방 모드")

    async def _handle_selection(
        self, interaction: discord.Interaction, mode: str, mode_kr: str
    ):
        if interaction.user.id != self.discord_user_id:
            await interaction.response.send_message(
                "❌ 본인의 알림만 선택할 수 있습니다.", ephemeral=True
            )
            return
        ...
        if self.discord_user_id in _active_user_tasks:
            ...
            if not prev_user_task.done():
                ...
                print(
                    f"[Task Cancelled] Aborted previous task for user {self.discord_user_id} due to new selection."
                )
        ...
        if self.video_no in _active_video_tasks:
            ...
            if not prev_task.done():
                prev_task.cancel()
                print(
                    f"[Task Cancelled] Aborted previous task for video {self.video_no} due to re-selection."
                )
...
async def execute_vod_pipeline_and_deliver(
    ...
    # Check user sandbox profiles if not found in global channel profiles
    if not solo_dict and not collab_dict and discord_user_id:
        user_profs = db.get_user_profiles(discord_user_id)
        for up in user_profs:
            if (
                up.get("profile_id") == target_dna
                or up.get("profile_name", "").lower() == target_dna.lower()
            ):
                solo_dict = up.get("solo_profile")
                collab_dict = up.get("collab_profile")
                break

    if not solo_dict and not collab_dict:
        print(f"[Error] No registered DNA profile found for '{target_dna}'")
        if status_channel:
            await status_channel.send(
                f"❌ **[프로필 미등록 오류]**\n"
                f"`{target_dna}` 채널의 발화 프로필이 존재하지 않습니다.\n"
                f"먼저 `/프로필등록` 명령어로 3+3 유튜브 분석을 완료해 주세요."
            )
        return False
    ...
    mode_kr = (
        ...
        else ("합방 모드" if selected_mode == "collab" else "전체 모드")
    )
        ...
        if status_msg:
            try:
                await status_msg.edit(
                    content="❌ **다시보기 분석 중 오류가 발생했습니다.** 다시 시도해 주세요."
                )
            except Exception:
                pass
        return False
    ...
    solo_xml = result.get("solo_xml_content", "") or result.get("solo_xml", "")
    solo_fcpxml = result.get("solo_fcpxml", "")
    collab_xml = result.get("collab_xml_content", "") or result.get("collab_xml", "")
    collab_fcpxml = result.get("collab_fcpxml", "")
    srt_content = result.get("srt_content", "") or result.get("subtitles_srt", "")
    ...
    if solo_xml:
        ...
        file_desc_lines.append("• Solo (XML): 프리미어/다빈치용 가편집 타임라인 파일")

    if solo_fcpxml:
        files_to_send.append(
            discord.File(
                io.BytesIO(solo_fcpxml.encode("utf-8")),
                filename=f"{b_date}_{streamer_name}_Solo_60fps.fcpxml",
            )
        )
        file_desc_lines.append(
            "• Solo (FCPXML): 파이널컷/다빈치 전용 FCPXML 시퀀스 파일"
        )
    ...
    if collab_fcpxml:
        files_to_send.append(
            discord.File(
                io.BytesIO(collab_fcpxml.encode("utf-8")),
                filename=f"{b_date}_{streamer_name}_Collab_60fps.fcpxml",
            )
        )
        file_desc_lines.append(
            "• Collab (FCPXML): 합방 다빈치/파이널컷 전용 FCPXML 시퀀스 파일"
        )

    speaker_srts = result.get("speaker_srts")
    if speaker_srts and isinstance(speaker_srts, dict):
        for fname, s_text in speaker_srts.items():
            if s_text.strip():
                files_to_send.append(
                    discord.File(
                        io.BytesIO(s_text.encode("utf-8")),
                        filename=fname,
                    )
                )
                file_desc_lines.append(
                    f"• {fname}: 화자별 분리 자막 파일 (개별 편집 가능)"
                )
    elif srt_content:
        files_to_send.append(
            discord.File(
        ...
        file_desc_lines.append(
            "• 자막 (SRT): 시퀀스 동기화 및 컷 경계 안전 클램핑된 칼싱크 자막"
        )
...
"""
    ...
    await outbound_queue.put(
        OutboundMessage(
            user_id=discord_user_id, content=delivery_msg, files=files_to_send
        )
    )
...
async def main():
    ...
    print(
        f"[Bot] Attempting to connect Discord bot with token prefix: {token[:10]}...",
        flush=True,
    )
