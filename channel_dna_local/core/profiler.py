...
class ChannelProfiler:
    ...
    def derive_two_track_profiles(
        ...
        from channel_dna_local.core.graph_engine import GraphEngine
    ...
    def calibrate_from_video_urls(
        ...
        import uuid
        pid = f"prof_{str(uuid.uuid4())[:8]}"

        from channel_dna_local.core.models import ChannelProfile
        
        solo_obj = ChannelProfile(
            profile_id=pid,
            channel_name=profile_name,
            avg_shot_length=solo_profile["avg_shot_length"],
            silence_tolerance=solo_profile["silence_tolerance"],
            highlight_rms_threshold=solo_profile["highlight_rms_threshold"],
            tension_interval=solo_profile["tension_interval"],
            motif_template=solo_profile["motif_template"],
            custom_vocab=solo_profile["custom_vocab"],
            speech_density_weight=solo_profile["speech_density_weight"],
            laughter_sensitivity=solo_profile["laughter_sensitivity"],
            sample_count=solo_profile["sample_count"],
            profile_type="solo",
