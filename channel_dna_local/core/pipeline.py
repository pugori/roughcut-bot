...
from channel_dna_local.core.models import (
    ChannelProfile,
    ProgressCallback,
    ScanMarker,
    VideoAnalysisResult,
...
class PipelineFacade:
    ...
    def audio_engine(self):
        if self._audio_engine is None:
            from channel_dna_local.core.audio_engine import AudioEngine
    ...
    def extractor(self):
        if self._extractor is None:
            from channel_dna_local.core.extractor import VideoExtractor
    ...
    def scanner(self):
        if self._scanner is None:
            from channel_dna_local.core.scanner import VODScanner
    ...
    def exporter(self):
        if self._exporter is None:
            from channel_dna_local.core.exporter import MarkerExporter
    ...
    def subtitle_engine(self):
        if self._subtitle_engine is None:
            from channel_dna_local.core.subtitles import SubtitleEngine
    ...
    def issue_collector(self):
        if self._issue_collector is None:
            from channel_dna_local.core.risk_engine import IssueDBCollector
    ...
    def risk_engine(self):
        if self._risk_engine is None:
            from channel_dna_local.core.risk_engine import RiskEngine
    ...
    def report_generator(self):
        if self._report_generator is None:
            from channel_dna_local.core.report_generator import ReportGenerator
    ...
    def rough_cut_renderer(self):
        if self._rough_cut_renderer is None:
            from channel_dna_local.core.rough_cut_renderer import RoughCutRenderer
    ...
    def guide_generator(self):
        if self._guide_generator is None:
            from channel_dna_local.core.guide_generator import GuideGenerator
    ...
    def llm_engine(self):
        if self._llm_engine is None:
            from channel_dna_local.core.llm_engine import LocalLLMEngine
        ...
        def _worker():
            try:
                ...
                if not prof_target or prof_target in ("기본 프로필", "Default"):
                    raise ValueError(
                        "선택된 채널 프로필이 없습니다. 먼저 상단 [프로필 추출] 탭에서 채널 DNA 프로필을 생성해 주세요."
                    )
                ...
                if not streamer_profile:
                    raise ValueError(
                        f"채널 프로필 '{prof_target}'을(를) 찾을 수 없습니다. 먼저 [프로필 추출]을 완료해 주세요."
                    )
        ...
        def _worker():
            try:
                ...
                if subtitles:
                    ...
                    if rough_subs:
                        self.subtitle_engine.export_srt_by_speakers(
    ...
    def export_streamer_package(
        ...
        """
        from channel_dna_local.core.utils import build_vod_folder_and_filenames
    ...
    def _fetch_vod_metadata(self, vod_url_or_no: str, streamer_name: str) -> VodMetadata:
        """Extracts video ID and fetches metadata from Chzzk."""
        from channel_dna_local.core.chzzk_client import extract_chzzk_video_no, fetch_chzzk_video_meta
    ...
    def _resolve_streamer_profiles(
        ...
        if not solo_p and not collab_p:
            raise ValueError(
                f"스트리머 '{target_streamer}'의 등록된 DNA 발화 프로필이 없습니다. 먼저 상단 [프로필 추출] 탭에서 프로필을 생성해 주세요."
            )
    ...
    def run_cloud_pipeline(
        ...
        from channel_dna_local.core.utils import sanitize_filename
        import tempfile
        from pathlib import Path
        ...
        try:
            ...
            audio_samples = self.scanner.audio_engine.extract_audio_in_memory(vod_url_or_no)
            ...
            if v_no and total_dur_sec > 0:
                try:
                    ...
                    preloaded_chats = self.scanner.chat_engine.fetch_vod_chat_logs(v_no, total_dur_sec)
            ...
            if subtitles:
                if solo_markers:
                    solo_subs = self.subtitle_engine.map_subtitles_to_rough_cut(
                        subtitles, solo_markers, fps=60.0
                    )
                if collab_markers:
                    collab_subs = self.subtitle_engine.map_subtitles_to_rough_cut(
            ...
            if target_subs:
                ...
                self.subtitle_engine.export_srt(target_subs, str(srt_path))
