...
export const BotTab: React.FC<BotTabProps> = ({
  ...
  const handleIssuePasscode = async () => {
    ...
    try {
      ...
      const copyText = `[제목] 안녕하세요 ${sName}님! 유튜브 영상을 즐겨보는 팬이 만든 작은 가편집 보조 봇 안내드립니다.

안녕하세요, ${sName}님! (그리고 편집자님/매니저님)
${sName}님의 유튜브 영상을 항상 재미있게 챙겨보고 있는 한 명의 팬이자 개발자입니다.

제가 생방송을 매번 시간 맞춰 챙겨보기는 어렵다 보니 주로 유튜브 편집 영상을 즐겨보곤 하는데요,
${sName}님과 편집자님의 긴 다시보기 컷 편집 수고를 조금이라도 덜어드리고 싶다는 마음에 개인적으로 치지직 자동 가편집 봇을 만들어 보았습니다.

생방송이 끝나면 다시보기 영상에서 불필요한 무음/대기 구간을 정리하여, 프리미어 프로나 파이널컷에서 바로 열어볼 수 있는 '1차 가편집 타임라인(XML)'과 '초벌 자막(SRT)'을 디스코드 DM으로 조용히 보내드리는 무료 팬 프로젝트 봇입니다.

혹시 편집 작업에 조금이라도 도움이 되실까 하여 ${sName}님 채널 전용으로 봇을 연동하실 수 있게 준비해 두었습니다.
부담 가지실 필요는 전혀 없으며, 편하실 때 가볍게 테스트 삼아 써보셔도 좋고 필요치 않으시면 편하게 넘기셔도 괜찮습니다!

──────────────────────────────────────────────────
■ 디스코드 봇 연결 방법
1. 아래 링크를 통해 작업용 디스코드 서버에 봇을 초대해 주세요.
   👉 봇 초대 링크: 
...
2. 봇과의 1:1 대화방(DM) 또는 서버 대화창에 아래 인증 명령어를 입력해 주세요.
   👉 /인증 암호:${passcode}

■ 어떻게 작동하나요?
• 방송이 종료되면 봇이 알아서 감지하여 가편집 시작 여부를 여쭤봅니다.
• [🎙️ 솔로 모드] 또는 [👥 합방 모드] 버튼을 누르시면, 잠시 후 타임라인 파일(XML)과 자막 파일(SRT)을 DM으로 배송해 드립니다.
• 지난 방송 링크(URL)를 봇 대화창에 그냥 붙여넣으셔도 바로 가편집 파일이 생성됩니다.
──────────────────────────────────────────────────

항상 좋은 영상과 방송 감사드리며, 조금이나마 창작 활동에 보탬이 되었으면 좋겠습니다.
환절기 건강 조심하시고 늘 응원하겠습니다!

${sName}님의 유튜브를 응원하는 팬 올림`;
